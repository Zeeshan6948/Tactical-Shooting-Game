...
public class AI : MonoBehaviour {
	...
	void Start(){
		...
		playerObj = LevelManager.m_instance.PlayerObject;
	...
	void OnEnable(){
		...
		PlayerWeaponsComponent = LevelManager.m_instance.PlayerObject.GetComponent<PlayerWeapons>();
...
//			dirPerpGizmo.Normalize();
		...
		while (true) {
			...
			if(attackTime < Time.time){
				if((!followPlayer || orderedMove) && Vector3.Distance(startPosition, myTransform.position) > pickNextDestDist){
					...
					SetSpeed(speedAmt);
                    if (this.GetComponent<RandomMotion>() != null && this.GetComponent<RandomMotion>().mover)
                    {
                        this.GetComponent<RandomMotion>().reached = true;
                    }
	...
	void CanSeeTarget(){
		...
		if((factionNum != 1 || playerAttacked) && FPSWalker.capsule){
			...
			if(!heardPlayer && !huntPlayer && FPSWalker.dropTime + 2.5f < Time.time){
				if(playerDistance < listenRange && (target == playerTransform || target == FPSWalker.transform || target == null)){
					if(PlayerWeaponsComponent && PlayerWeaponsComponent.CurrentWeaponBehaviorComponent){
						...
						if(WeaponBehaviorComponent.shootStartTime + 2.0f > Time.time && !WeaponBehaviorComponent.silentShots){
							if(target == FPSWalker.transform){
								targetEyeHeight = 0.0f;
								target = FPSWalker.transform;
								pursueTarget = true;
							}else{
			...
			if(playerDistance < attackRangeAmt){
				...
				if((Mathf.Abs(FPSWalker.leanAmt) < 0.1f || playerDistance < 2.0f) && target == FPSWalker.transform){
					targetEyeHeight = FPSWalker.capsule.height * 0.25f;
					target = playerTransform;
				}
		...
		if(target == playerTransform || target == FPSWalker.transform || (TargetAIComponent && TargetAIComponent.enabled && target != null)){
	...
	IEnumerator AttackTarget(){
		while (true) {
			...
			if(!huntPlayer){
				...
				if(heardPlayer && (target == playerTransform || target == FPSWalker.transform)){
					InitializeAnim();
					speedAmt = runSpeed;
			...
			if(pursueTarget){//should NPC attack player collider or leaning collider?
				lastVisibleTargetPosition = FPSWalker.transform.position;
			}else{
				lastVisibleTargetPosition = target.position + (target.up * targetEyeHeight);
			}
			...
			if(targetVisible){
				...
				if(distance > shootRange){
					...
					if(!turning){
						...
						StartCoroutine(RotateTowards(lastVisibleTargetPosition, 20.0f, 0.5f));
					}
					speedAmt = 0.0f;
					SetSpeed(speedAmt);
	...
	void SearchTarget( Vector3 position  ){
		if(attackFinished){
			if(target == playerTransform || target == FPSWalker.transform || (TargetAIComponent && TargetAIComponent.enabled)){
				if(!huntPlayer){
					speedAmt = runSpeed;
					TravelToPoint(target.position);
				}
	...
	public IEnumerator RotateTowards( Vector3 position, float rotationSpeed, float turnTimer, bool attacking = true ){
		...
		while(turnTime + turnTimer > Time.time && !cancelRotate){
			...
			if(pursueTarget){
				position = FPSWalker.transform.position;
			}else{
	...
	public void CommandNPC () {
		if(factionNum == 1 && followOnUse && commandedTime + 0.5f < Time.time){
			...
			if(attackFinished && !turning){
				StopCoroutine("RotateTowards");
				StartCoroutine(RotateTowards(playerTransform.position, 10.0f, 0.5f, false));
			}
			if(!followPlayer){
